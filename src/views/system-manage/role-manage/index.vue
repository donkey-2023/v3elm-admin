    ...
    <div ref="tableWrapRef" class="table-wrap">
      ...
      >
        <table-column-wrap ref="columnWrapRef" :key="$store.getters.key">
          ...
          <el-table-column prop="roleVal" label="角色值" align="center" />
...
const total = ref(0)
const query = () => {
  $http.post('/loadRolesInfo', dataForm).then(res => {
    arr = res.data || []
    total.value = arr.length
    tableData.value = arr.filter(item => {
      const condition1 = dataForm.roleName ? item.roleName.indexOf(dataForm.roleName) > -1 : true
